...
const PetProjectCard: React.FC<PetProjectCardProps> = ({ data }) => (
  ...
  >
    ...
    <div className="flex flex-col mb-10">
      ...
      {/** title & description */}
      <div>
        <h3 className="text-lg font-bold mb-2 md:text-2xl md:mb-4">
          {data.name}
        </h3>
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap gap-2">
        {data.languages.map((language: string) => (
          <span className="bg-white px-3 py-[9px] rounded-lg text-darkblue text-center">
            {language}
          </span>
        ))}
      </div>
      <div className="hidden md:flex md:flex-wrap md:gap-2">
        {data.topics.map((topic: string) => (
          <span className="bg-lightgray px-3 py-[9px] rounded-lg text-darkblue text-center">
            {topic}
          </span>
        ))}
      </div>
