    ...
    <div className="flex flex-col mb-10">
      ...
      <div className="flex justify-between items-center gap-4 mb-4 sm:mb-6">
        <div className="flex items-center gap-2 bg-white rounded-lg text-darkblue pl-3 pr-9 py-2">
          <img
            src="../../../../public/icons/landing/exclamation-mark.svg"
      ...
      {/** title & description */}
      <div>
        <h3 className="text-lg font-bold mb-2 sm:text-2xl sm:mb-4">
          {data.name}
        </h3>
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap gap-2">
        {data.languages.map((language: string, index: number) => (
          <span
            key={index}
            className="bg-white px-3 py-[9px] rounded-lg text-darkblue text-center"
          >
            {language}
          </span>
        ))}
      ...
      <div className="hidden sm:flex sm:flex-wrap sm:gap-2">
        {data.topics.map((topic: string, index: number) => (
          <span
            key={index}
            className="bg-lightgray px-3 py-[9px] rounded-lg text-darkblue text-center"
          >
            {topic}
          </span>
        ))}
    ...
    <div>
      ...
      <div className="flex gap-1 sm:hidden">
        ...
        <span
          className={cn(
            "block rounded-full w-2 h-2",
            [PetProjectDifficulty.MEDIUM, PetProjectDifficulty.HARD].some(
              (d) => difficulty === d,
            )
              ? "bg-yellow"
              : "bg-white",
